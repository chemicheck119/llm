...
전국 출동 위치·경로와 실제 AI 도구 실행 상태를 연결하는 방법은
[전국 현장대응 에이전트와 지도 연동](docs/OPERATIONS_AGENT_AND_MAP.md)에서 가장 쉽게
확인할 수 있습니다.
...
| 전국 범위 감사 | 구현 | 배포 후보 기준 17개 시·도·28,647개 시설·168,424개 과거 이력 후보 |
| 현장대응 에이전트 | 구현 | 10단계 실행 상태·다음 행동·사고/현재 위치·서버 경로 계약 |
| 물질 충돌 검토 | 공개 근거 파일럿 | 공개 검증 crosswalk CAS 6개·물질쌍 15개, pair별 표시 계약, `expert_reviewed=false` |
| 공식근거 교차검증 | 구현 | 2쌍은 독립 공식기관 3곳 이상 교차확인, 나머지 13쌍은 CAMEO 단일체계로 구분 |
...
| 현장대응 에이전트 | 파서·검색·확인·규칙의 실제 진행 상태를 보고 다음 안전 행동을 선택하는 결정론적 상태 머신 |
