...
> **소방안전 빅데이터 플랫폼의 화학사고 표현과 물성 데이터를 AI가 학습해, 출동 중 불완전한
> 신고에서 물질 후보를 찾고 확인된 공식근거만으로 충돌 위험을 검토합니다.**

[`제6회 소방안전 빅데이터 활용 및 아이디어 경진대회`](https://www.bigdata-119.kr/bbs/view?bbsctt_id=571)
`서비스 개발 부문` · `공모전 시연 가능` · `FastAPI·Docker 구현` · `서울 Cloud Run preview 배포`

케미체크119는 [소방안전 빅데이터 플랫폼](https://www.bigdata-119.kr/)의 데이터를 단순 조회하거나
화면에 나열하는 서비스가 아닙니다. 플랫폼의 과거 화학사고 물질 표현을 후보 검색 모델에
학습하고, 공개 물성 정보를 “현장에서 무엇을 더 확인해야 하는가”를 제시하는 물질 탐색 기능으로
가공했습니다. ICIS·PRTR, KOSHA MSDS, NOAA CAMEO를 결합해 신고 접수부터 물질 확인, 충돌
검토, 근거 제시까지 하나의 태블릿 흐름으로 연결합니다.

하나의 거대한 LLM이 화학 반응을 추측하지 않습니다. 검색 모델·현장 확인 gate·CAMEO 규칙·
근거 검증·선택형 LLM을 역할별로 분리한 **Evidence-Gated Hybrid Incident Agent**입니다.

> 공모전 시연 상태는 `READY_WITH_DISCLOSED_LIMITATIONS`입니다. 공개 데이터 기반 의사결정
> 보조 시스템이며, 실제 현장 운영·지휘 시스템으로 승인된 상태는 아닙니다.
...
| 공모전 제출 관점 | 내용 |
|---|---|
| 제안 분야 | AI·데이터 기술을 활용한 소방 현장 적용형 웹 서비스 |
| 핵심 데이터 | 소방안전 빅데이터 플랫폼의 화학물정보·화학사고별 유해물질판단 |
| 핵심 사용자 | 출동 중인 소방대원·현장 지휘관 |
| 핵심 차별성 | 모르면 기권하고, 확인된 CAS 두 개와 공식근거가 있을 때만 충돌 검토 |
| 구현 결과 | 모델 pipeline·FastAPI·Agent·RAG·Docker·Cloud Run·FE/BE 계약 |
...
## 2. 소방안전 빅데이터 플랫폼을 어떻게 활용했나

플랫폼 데이터는 프로젝트의 출발점이자 두 AI 기능의 직접 입력입니다.

| 플랫폼 데이터 | 원천 규모 | 실제 가공 결과 | 서비스 기능 |
|---|---:|---:|---|
| 울산소방 화학물정보 | 4,378행 | 카탈로그와 연결된 물성 프로필 749 CAS | 물질명을 모를 때 상태·색상·냄새·용도로 후보 탐색 |
| 화학사고별 유해물질판단 | 1,868행 | 유효 물질명–CAS 표현 1,530건, 신규 학습 표현 241개 | 현장식 표현·띄어쓰기 차이를 반영한 Resolver source adaptation |
| 2020년 미래 구간 | 419개 표현 | Top-1 0.3246 → 0.6706 | 과거 소방 사고 표현 학습 효과 검증 |

```mermaid
flowchart LR
    A["소방안전 빅데이터 플랫폼"] --> B["화학사고 물질명·CAS"]
    A --> C["상태·색상·냄새·용도"]
    B --> D["현장 표현 Resolver 학습"]
    C --> E["물질 Discovery 프로필"]
    D --> F["신고문 물질 Top-K 후보"]
    E --> G["다음 현장 확인 질문"]
    F --> H["태블릿 대응충돌검토"]
    G --> H
...
플랫폼 데이터가 없다면 표준 화학물질명 검색에 머뭅니다. 플랫폼 데이터를 적용하면 실제
소방 사고 기록에서 사용된 표현을 검색 모델이 기억하고, 물질명을 모르는 상황에서도 공개
물성으로 후보를 좁힐 수 있습니다. 이 연결이 케미체크119의 공모전 데이터 활용 핵심입니다.

- 플랫폼 원천 1: [울산광역시소방본부 화학물 정보](https://www.bigdata-119.kr/goods/goodsInfo?goods_mng_sn=21)
- 플랫폼 원천 2: [울산 화학사고별 유해물질 판단 정보](https://www.bigdata-119.kr/goods/goodsInfo?goods_mng_sn=5)
- 학습·시간 분할 검증: [`src/chemiguard119/incident_adaptation.py`](src/chemiguard119/incident_adaptation.py)
- 물성 기반 탐색: [`src/chemiguard119/discovery.py`](src/chemiguard119/discovery.py)
- 데이터·모델 상세: [데이터와 모델 문서](docs/DATA_AND_MODEL.md)

위 Top-1 향상은 동일한 소방 원천을 연도순으로 나눈 표현 검색 평가입니다. 전국 현장 정확도나
사고 대응 성공률로 해석하지 않습니다.
...
| 심사 항목 | 케미체크119의 대응 | 확인 가능한 구현 증거 |
|---|---|---|
| 플랫폼 활용성 20점 | 소방 플랫폼 원천을 Resolver 학습과 물성 Discovery에 직접 사용 | 1,868행 사고–CAS 학습, 749 CAS 물성 프로필, 데이터 manifest·checksum |
| 창의성 20점 | 답을 생성하는 챗봇이 아니라 증거가 충족될 때만 도구를 실행하는 Agent | `PLAN → ACT → OBSERVE → REPLAN`, confirmation gate, Grounded RAG |
| 문제해결성 20점 | 출동 중 비정형 신고·시설물질 정보 공백·혼합 위험 확인 문제를 한 흐름으로 해결 | 통합 사고분석 API, 업체 과거 이력 후보, 현장 확인 상태 전이 |
| 실현가능성 20점 | CPU 우선 FastAPI·Docker 서비스와 FE/BE 계약을 구현해 실제 서버에 배포 | 자동 테스트 426개, CI Docker build, 서울 Cloud Run Blue/Green smoke |
| 기대효과 20점 | 전국 확장 가능한 후보 검색과 대응기록 축적으로 출동 중 탐색 부담 완화 | 17개 시·도·28,647개 시설 범위, 구조화 응답·기록저장 BFF 계약 |
...
대표 사용 시점은 신고 접수 직후부터 현장 도착 전까지의 이동 시간입니다.

```text
상황실 신고 접수
→ 출동 지령과 신고문을 MDT·태블릿에서 확인
→ Agent가 물질·시설 후보와 공식근거를 병렬 탐색
→ 대원은 이동 중 확인할 항목을 미리 파악
→ 현장에서 라벨·MSDS·운송문서로 두 물질 확인
→ 확인된 CAS 조합만 충돌 규칙 실행
→ 근거·판정·확인 기록을 하나의 대응기록으로 저장
```

[태블릿 대시보드 디자인](https://www.figma.com/make/APHOgF5QNcsoucHMflVkY8/%EC%86%8C%EB%B0%A9-%EB%8C%80%EC%8B%9C%EB%B3%B4%EB%93%9C-%EB%94%94%EC%9E%90%EC%9D%B8?p=f&t=7kbKZoOyhlZz0tWB-0)은 지도와 AI 대응 패널을 함께 보여주며, 물질 후보를 확정 정보와 구분하고
공식근거를 접어서 확인할 수 있도록 설계했습니다.
...
flowchart LR
    ...
    C --> C2["설명 가능한 후보 Ranker·다음 확인 정책"]
    B --> D["ICIS·PRTR 시설 과거 이력"]
    C2 --> E["KOSHA·CAMEO 하이브리드 검색"]
    D --> F["현장 확인 Gate"]
    E --> F
...
|---|---|---|
| 신고문 구조화 | 사전·정규식·결정적 파서, 선택형 LLM | 물질 표현·사고유형·부정 표현 추출; 위치·시설은 구조화 입력으로 보완 |
...
| 물질 후보 Ranker | 기존 검색 사전값 + 식별·출처·물성·공식근거 특징 | 후보 순위를 설명하고 정보이득 기반 다음 현장 확인 행동 선택 |
...
| 물성 자기검색 회귀 감사 | 300건 | BM25 Top-1 0.6933 유지, 다음 구분 확인 생성 77.33%; 현장 정확도 아님 |
...
1. Parser      → 사고물질·누출 유형 후보 추출(시설·위치는 구조화 입력 사용)
