...
| 사고 분석 E2E | 부분 완료 | 8건 DRAFT 회귀 + 정답 없는 50건 안전 preflight, 사람 독립 검수는 미실시 |
...
내부 Resolver·Retriever·E2E 평가 데이터는 작으므로 회귀 방지용입니다. 파서의 전국 442건은
다른 기관·미래 연도 외부 감사이지만 CAS 정답과 실제 신고 음성 전사가 없어 현장 정확도를
의미하지 않습니다.
...
공모전 발표·시연은 [공모전 공식근거 보증 기준](COMPETITION_ASSURANCE.md)에 따라 가능하지만,
다음 항목은 실제 현장 파일럿과 운영 배포를 계속 차단합니다.
...
8. 관찰 기반 탐색은 300건 source-derived 자기검색 회귀가 있지만 독립 오인·거부 평가셋은 없습니다.
...
1. parser는 전국 공식 사고 442건 외부 보류 감사를 추가했지만 실제 신고 음성 전사·CAS 정답은
   없습니다. Resolver 21건, legacy retrieval 10건과 section retrieval 12건은 여전히 내부
   DRAFT이므로 독립 보류셋을 새로 구축해야 합니다.
...
4. 소방 사고 물질명–CAS source adaptation은 완료됐지만 처음 보는 표현·CAS 성능은 개선되지
   않았습니다. 생성형 신고문 파서 파인튜닝은 실제 전사·개체 라벨 부족으로 아직 부적합합니다.
...
1. FE·BE가 물질탐색·사고분석 계약을 구현하고 배포된 BFF를 대상으로 contract smoke를
   실행합니다.
2. 공모전에서는 공식근거 보증 상태와 `expert_reviewed=false`를 FE 카드에 그대로 표시합니다.
3. 실제 파일럿을 추진할 때만 서로 다른 두 사람이 E2E 50건을 독립 라벨링하고, 이후
   200건 이상으로 확장해 실제 E2E 성능을 검증합니다.
4. CAMEO·ICIS 파생 데이터의 재배포 조건을 확인해 registry 승인을 기록합니다.
