...
### 8.3.1 설명 가능한 물질 후보 Ranker

`src/chemiguard119/material_ranker.py`는 기존 검색 순위, 이름 유사도, 정확 식별, 별칭
출처, 물성 일치 범위, 동일 CAS 공식근거 적재 여부를 고정 가중치로 결합합니다. 현재는 검수된
질의-CAS 라벨이 부족해 지도학습 모델로 가장하지 않으며
`NOT_SUPERVISED_INSUFFICIENT_REVIEWED_LABELS`를 API에 공개합니다.

기존 FTS5 BM25 순위를 강한 사전값으로 유지하고 정확 CAS·별칭 근거가 있을 때만 재정렬합니다.
후보가 여러 개면 아직 관찰되지 않은 상태·색상·용도·냄새 필드 중 후보 구분력이 높은 항목을
정보이득 정책으로 선택합니다. 냄새 확인은 직접 맡는 행동을 금지하고 기존 신고·계측기·MSDS
확인만 안내합니다. Ranker 점수와 구분력 점수는 모두 확률이 아닙니다.
