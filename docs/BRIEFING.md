...
기준일: 2026-08-01
발표용 결론: **서울 Cloud Run 공모전 preview 시연 가능, 공식기관 다중근거 자동검증 적용,
실제 현장 운영은 검증·라이선스 gate가 의도적으로 차단**
...
대표 차아염소산나트륨–염산 위험은 CAMEO·CDC 사고보고·CDC 지침·ILO/WHO ICSC·UKHSA의
5개 자료, 4개 독립기관으로 교차증빙합니다. 금속 나트륨–염산의 수소·화재·폭발 위험도
CAMEO·ILO/WHO ICSC·OSHA의 3개 독립기관으로 확인합니다. 이를 **공식근거 교차확인**으로
표시하되 사람 전문가 승인으로 부르지 않습니다. 다른 13개 조합은 CAMEO 단일체계 상태를
그대로 공개합니다.
...
| 사고 표현 적응학습 | train 2015~2019 / locked test 2020 419건 | 위험판정이 아닌 물질명→CAS 후보 검색만 학습 |
| 생성형 신고문 파인튜닝 승인 데이터 | 0건 | 실제 전사·개체 라벨 부족으로 QLoRA 보류 |
...
소방안전 빅데이터의 `화학사고별 유해물질판단` 1,868행에서 유효한 단일 CAS와 비모호
물질 표현만 추출했습니다. 2015~2019년 표현으로 기존 Resolver를 적응 학습하고 2020년
419건은 학습에 넣지 않은 잠금 테스트로 남겼습니다.

```text
2020 Top-1: 0.3246 → 0.8974
2020 Top-3: 0.3461 → 0.9021
평가 CAS의 artifact 포함률: 0.7422 → 0.9714
잘못된 단일 CAS 확정: 0건 → 0건
처음 보는 표현 60건 Top-1: 0.2833 → 0.2833
```

기본 카탈로그 밖 35개 CAS·113개 표현은 소방 원천의 정확한 과거 표현에만 후보로 노출하고
문자 유사도 추정에서는 제외했습니다. 즉, 공개 소방 기록에서 반복되는 현장 표현을 더 잘
찾게 됐지만 새로운 표현까지 이해하는 범용 언어모델이 된 것은 아닙니다. 결과는 계속 후보이며
현장 확인 전 충돌 규칙에 넣지 않습니다.

### 생성형 LLM QLoRA를 하지 않은 이유
...
6개 DRAFT 신고문을 증식해 QLoRA하면 문장 패턴을 외우는 모델은 만들 수 있지만 일반화
성능은 증명할 수 없습니다. 그래서 실제 CAS 정답이 있는 공개 기록에는 Resolver 적응학습을
실행하고, 정답 라벨이 없는 신고문 생성형 파인튜닝은 보류했습니다.
...
75.8%이고 답변 불가 사례도 0건입니다. 사람 독립검수 인력을 확보하지 못했으므로 50건
후보를 정확도 근거로 바꾸지 않았습니다. 공모전에서는 별도의
[공식근거 보증 기준](COMPETITION_ASSURANCE.md)으로 공식기관 교차확인·안전 gate·출처
생존·배포 상태를 각각 증명하며, 독립 locked set 전에는 실제 현장용 staging·production을
계속 차단합니다.

**“왜 최신 LLM을 파인튜닝하지 않았나요?”**
아무것도 학습하지 않은 것이 아닙니다. CAS 정답이 있는 소방 사고 기록에는 Resolver
적응학습을 실제 수행해 2020 잠금셋 Top-1을 0.3246에서 0.8974로 높였습니다. 반면 실제
신고 전사와 검수된 개체 라벨은 0건이라 생성형 QLoRA 수치를 만들면 과적합을 성능으로
포장하게 됩니다. LLM은 안전 판정자가 아니라 신고 구조화·근거 요약 보조로 제한합니다.
...
**“지금 상용화할 수 있나요?”**
공모전 preview는 서울 Cloud Run에 배포되어 시연할 수 있습니다. 다만 이것은
`READY_WITH_DISCLOSED_LIMITATIONS`이며, 사람 전문가 승인이나 현장 정확도 검증이 아닙니다.
실제 운영은 현장 성능, 독립 locked 평가, 데이터 재배포 권리와 FE·BE 스테이징 연동이
완료될 때까지 차단됩니다.
